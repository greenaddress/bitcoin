...
// Fedora does not include NID_secp256k1 in openssl due to patent concerns. As a
// (rather unsatifactory) workaround, implement the curve here to avoid having to
// rebuild/replace openssl on fedora.
//
// THIS WORKAROUND SHOULD NOT BE SUBMITTED UPSTREAM
//
#define FEDORA_WORKAROUND

#ifdef FEDORA_WORKAROUND
typedef struct {
...
{
    return ec_group_new_from_data(&_EC_SECG_PRIME_256K1.h);
}
